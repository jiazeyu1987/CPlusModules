...
QFileInfoList Dicom::fileInfoList;
    ...
    PatientsSeries& patientsSequenceList)
{
  ...
  if (entryvaluePatientName)
  {
    ...
    std::cout << "Patient's Name(" << entryIdPatientName << ") ";
  ...
  if (entryvaluePatientID)
  {
    ...
    std::cout << "Patient's ID(" << entryIdPatientID << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvaluePatientBirthDate)
  {
    ...
    std::cout << "Patient's BirthDate(" << entryIdPatientBirthDate << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvaluePatientSex)
  {
    ...
    std::cout << "Patient's Sex(" << entryIdPatientSex << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  { 
    ...
    std::cout << "Patient's Modality(" << entryIdModality << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvalueAccessionNumber)
  {
    ...
    std::cout << "Patient's AccessionNumber(" << entryIdAccessionNumber << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvalueSeriesNumber)
  {
    ...
    std::cout << "Patient's SeriesNumber(" << entryIdSeriesNumber << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvalueSeriesDate)
  {
    ...
    QDate dt = QDate::fromString(QString::fromStdString(tagvalue), "yyyyMMdd");  
    datetime.setDate(dt);
    std::cout << "Patient's SeriesDate(" << entryIdSeriesDate << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvalueSeriesTime)
  {
    ...
    datetime.setTime(dt);
    std::cout << "Patient's SeriesTime(" << entryIdSeriesTime << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvalueSeriesDescription)
  {
    ...
    std::cout << "Patient's SeriesDescription(" << entryIdSeriesDescription << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvalueSeriesInstanceUID)
  {
    ...
    std::cout << "Patient's Series Instance UID(" << entryIdSeriesInstanceUID << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
  ...
  if (entryvaluePixelSpacing)
  {
    ...
    std::cout << "Patient's PixelSpacing(" << entryIdPixelSpacing << ") ";
    std::cout << " is: " << tagvalue << std::endl;
  }
  else
  {
...
void Dicom::itkReadDicomSeriesAnyDir(QString path)
{
    ...
    progressBar.setText(QStringLiteral("正在导入影像文件，请稍后……"));
    ...
    //QString missTxt;
    //if (missList.size() > 0)
    //    missTxt = QStringLiteral("下列影像文件缺失：\n%1").arg(missList.join("\n"));
    ...
    //        .arg(missTxt, dcmTxt);
    //    msg.queRenWarn(QStringLiteral("患者管理"), dicomBadTxt);
    //}
}
...
int Dicom::itkReadDicomSeries(std::string dirName, QStringList& badFileList, QVector<struct Miss>& missVec)
{
    ...
    catch (itk::ExceptionObject & ex)
    {
        std::cout << ex << std::endl;
        GLMessageBox msg;
        msg.queRenWarn(QStringLiteral("患者管理"), QStringLiteral("读取序列文件夹错误:") + ex.what());
        badFileList << totalList;
        return EXIT_FAILURE;
    }
    badFileList << totalList;
    ...
    PatientsSeries& series, QString moveDir, QVector<int>& missVec_)
{
    ...
    catch (itk::ExceptionObject& ex)
    {
        std::cout << ex << std::endl;
        GLMessageBox msg;
        msg.queRenWarn(QStringLiteral("患者管理"), QStringLiteral("单个序列读取错误:") + ex.what());
        return EXIT_FAILURE;
    }
    ...
    //字符编码
    tagItrEncode;
    MetaDataStringType::ConstPointer entryvalueEncode = nullptr;
    if (tagItrEncode == end)
    {
        std::cerr << "Tag " << entryIdEncode;
        std::cerr << " not found in the DICOM header" << std::endl;
        //return EXIT_FAILURE;
    }
    else
    {
        entryvalueEncode = dynamic_cast<const MetaDataStringType*>(tagItrEncode->second.GetPointer());
    }
    ...
    if (entryvalueEncode)
    {
        encode = entryvalueEncode->GetMetaDataObjectValue();
    }
    else
    {
        std::cerr << "Entry was not of string type" << std::endl;
    ...
    if (entryvaluePatientName)
    {
        std::string tagvalue = entryvaluePatientName->GetMetaDataObjectValue();
        patient.patientName = QString::fromStdString(tagvalue).trimmed();
        QString encode_ = QString::fromStdString(encode).trimmed();
        if (encode_.size() > 0)
        {
            QTextCodec* codec = QTextCodec::codecForName(encode_.toStdString().c_str());
            patient.patientName = codec->toUnicode(tagvalue.data());
        }
    ...
    if (entryvalueThickness)
    {
        ...
        std::cout << "Patient's Thickness" << entryIdThickness << ") ";
        std::cout << " is: " << tagvalue << std::endl;
    }
    else
    {
    ...
    if (entryvalueSpaceBetween)
    {
        ...
        std::cout << "Patient's Space Between" << entryIdSpaceBetween << ") ";
        std::cout << " is: " << tagvalue << std::endl;
    }
    else
    {
    ...
    if (!moveDir.isEmpty()) //判断是否移动数据库
    {
        ...
        GLTool().createPath(series.seriesDir, Dir);
        // 移动之前生成缩略图
    ...
    {
        series.seriesDir = QFileInfo(QString::fromStdString(source)).absolutePath(); // 
        QString target = series.seriesDir + "/cover.png";
        QString screenshot = itkDicomSaveToPng(source, target.toStdString());
        series.screenshot = screenshot; //缩略图
    }
...
QString Dicom::itkDicomSaveToPng(std::string source, std::string target)
{
    ...
    {
        std::cerr << "itkDicomSaveToPng: " << ex.what() << std::endl;
        return QString();
    }

    return QString::fromLocal8Bit(target.c_str());
}
...
int Dicom::dicomSequenceErrorAndSeriesMiss(std::string& fileName)
{
    ...
    if (entryvalueSeriesSeq)
    {
        ...
        std::cout << "Patient's SeriesSeq(" << entryIdSeriesSeq << ") ";
        std::cout << " is: " << tagvalue << std::endl;
...
void Dicom::ShowMissWarning(QVector<struct Miss> missVec, QStringList badFileList)
{
    ...
    if (!missTxt.isEmpty() || !dcmTxt.isEmpty())
    {
        ...
        QString dicomBadTxt = QStringLiteral("%1%2")
            .arg(missTxt, dcmTxt);
        qGLMessageDialog::ShowInformation(QStringLiteral("患者管理"), dicomBadTxt);
    }
}
