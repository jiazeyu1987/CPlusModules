...
#ifndef __qSlicerNaviFrameWidget_h
#define __qSlicerNaviFrameWidget_h

// Segmentations includes
#include "qSlicerNaviModuleWidgetsExport.h"
...
class qSlicerNaviFrameWidgetPrivate;
...
///
class Q_SLICER_MODULE_NAVI_WIDGETS_EXPORT qSlicerNaviFrameWidget : public qMRMLWidget
{
    Q_OBJECT
        QVTK_OBJECT
...
public:
    ...
    /// Constructor
    explicit qSlicerNaviFrameWidget(QWidget* parent = nullptr);
    /// Destructor
    ~qSlicerNaviFrameWidget() override;
    ...
    //@by jzy
    vtkMRMLSegmentationNode* AddNewSegmentationNode(QString label);
    void SetMasterNode(vtkMRMLNode* node);
...
public slots:
    ...
    void setClonedNode(vtkMRMLNode* node);

    vtkMRMLNode* getClonedNode();
...
public:
    QScopedPointer<qSlicerNaviFrameWidgetPrivate> d_ptr;
private:
    Q_DECLARE_PRIVATE(qSlicerNaviFrameWidget);
    Q_DISABLE_COPY(qSlicerNaviFrameWidget);
};
