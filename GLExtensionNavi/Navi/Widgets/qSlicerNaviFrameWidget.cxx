...
#include "qSlicerNaviFrameWidget.h"
#include "qMRMLSortFilterSegmentsProxyModel.h"
#include "ui_qSlicerNaviFrameWidget.h"
...
public:
    ...
    QPointer<qSlicerNaviFrameWidget> EditorWidget;
...
//-----------------------------------------------------------------------------
// qSlicerNaviFrameWidgetPrivate methods

//-----------------------------------------------------------------------------
class qSlicerNaviFrameWidgetPrivate : public Ui_qSlicerNaviFrameWidget
{
    Q_DECLARE_PUBLIC(qSlicerNaviFrameWidget);

protected:
    qSlicerNaviFrameWidget* const q_ptr;
public:
    qSlicerNaviFrameWidgetPrivate(qSlicerNaviFrameWidget& object);
    ~qSlicerNaviFrameWidgetPrivate();
    void init();
...
public:
    ...
    vtkMRMLNode* ClonedNode = nullptr;
    ...
    /// Ordering of effects
    QStringList EffectNameOrderBasic;
    QStringList EffectNameOrderAdvance;
    ...
    QVector<QGroupBox*> EffectsGroups;
    QMap<QString, QString> EffectNameMapper;
};

//-----------------------------------------------------------------------------
qSlicerNaviFrameWidgetPrivate::qSlicerNaviFrameWidgetPrivate(qSlicerNaviFrameWidget& object)
    : q_ptr(&object)
    , Locked(false)
    ...
    , RotateWarningInNodeSelectorLayout(true)
{
    ...
    this->EffectNameOrderBasic
        ...
        << "Threshold"
        // Local painting
        << "Paint" << "Draw" << "Erase" << "Scissors"

        <<"Flood filling";
        //need more than one segment
        //<< "Grow from seeds" << "Fill between slices"
        // Global processing
    this->EffectNameOrderAdvance
        << "Margin" << "Hollow" 
        // Global splitting, merging
        << "Islands";
   EffectNameMapper["None"] = QStringLiteral("默认");
   EffectNameMapper["Threshold"] = QStringLiteral("阈值");
   EffectNameMapper["Paint"] = QStringLiteral("画笔");
   EffectNameMapper["Draw"] = QStringLiteral("钢笔");
   EffectNameMapper["Flood filling"] = QStringLiteral("生长");
   EffectNameMapper["Erase"] = QStringLiteral("删除单层");
   EffectNameMapper["Scissors"] = QStringLiteral("删除体积");;

   EffectNameMapper["Margin"] = QStringLiteral("增大缩小");
   EffectNameMapper["Hollow"] = QStringLiteral("空洞");
   EffectNameMapper["Smoothing"] = QStringLiteral("平滑");
   EffectNameMapper["Islands"] = QStringLiteral("孤岛");

   EffectNameMapper["Level tracing"] = QStringLiteral("边界追踪");
   EffectNameMapper["Mask volume"] = QStringLiteral("序列遮罩");
   EffectNameMapper["Draw tube"] = QStringLiteral("牵引管");
   EffectNameMapper["Engrave"] = QStringLiteral("雕刻");
   EffectNameMapper["Fast Marching"] = QStringLiteral("快速匹配");
   EffectNameMapper["Local Threshold"] = QStringLiteral("高级阈值");
   EffectNameMapper["Split volume"] = QStringLiteral("序列分割");
   EffectNameMapper["Surface cut"] = QStringLiteral("表面切割");
   EffectNameMapper["Watershed"] = QStringLiteral("转折点");
   EffectNameMapper["Logical operators"] = QStringLiteral("逻辑运算");
   EffectNameMapper["Fill between slices"] = QStringLiteral("层间填充");
   EffectNameMapper["Grow from seeds"] = QStringLiteral("层间成长");
    this->UnorderedEffectsVisible = true;
    this->DefaultTerminologyEntrySettingsKey = "Segmentations/DefaultTerminologyEntry";
}

//-----------------------------------------------------------------------------
qSlicerNaviFrameWidgetPrivate::~qSlicerNaviFrameWidgetPrivate()
{
    Q_Q(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidgetPrivate::init()
{
    Q_Q(qSlicerNaviFrameWidget);
    this->setupUi(q);

    EffectsGroups = {
       this->EffectsGroupBasic,
       this->EffectsGroupAdvance,
       this->EffectsGroupResearch
    };
    ...
    this->MaskModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Everywhere"), vtkMRMLSegmentEditorNode::OverwriteAllSegments);
    this->MaskModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Inside all segments"), vtkMRMLSegmentEditorNode::OverwriteVisibleSegments);
    this->MaskModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Inside all visible segments"), vtkMRMLSegmentEditorNode::OverwriteNone);
    this->MaskModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Outside all segments"), vtkMRMLSegmentEditorNode::Overwrite_Last);
    ...
    this->OverwriteModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Overwrite all"), vtkMRMLSegmentEditorNode::OverwriteAllSegments);
    this->OverwriteModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Overwrite visible"), vtkMRMLSegmentEditorNode::OverwriteVisibleSegments);
    this->OverwriteModeComboBox->addItem(qSlicerNaviFrameWidget::tr("Allow overlap"), vtkMRMLSegmentEditorNode::OverwriteNone);

    this->SwitchToSegmentationsButton->setIcon(q->style()->standardIcon(QStyle::SP_ArrowRight));

    QMenu* segmentationsButtonMenu = new QMenu(qSlicerNaviFrameWidget::tr("Segmentations"), this->SwitchToSegmentationsButton);
    ...
    QMenu* show3DButtonMenu = new QMenu(qSlicerNaviFrameWidget::tr("Show 3D"), this->Show3DButton);

    this->SurfaceSmoothingEnabledAction = new QAction(qSlicerNaviFrameWidget::tr("Surface smoothing"), show3DButtonMenu);
    this->SurfaceSmoothingEnabledAction->setToolTip(qSlicerNaviFrameWidget::tr("Apply smoothing when converting binary lablemap to closed surface representation."));
    ...
    QMenu* surfaceSmoothingFactorMenu = new QMenu(qSlicerNaviFrameWidget::tr("Smoothing factor"), show3DButtonMenu);
    ...
    this->SurfaceSmoothingSlider->setToolTip(qSlicerNaviFrameWidget::tr("Higher value means stronger smoothing during closed surface representation conversion."));
    ...
    this->AddSegmentButton->setEnabled(true);
    this->RemoveSegmentButton->setEnabled(true);
    this->Show3DButton->setEnabled(true); 
    this->SwitchToSegmentationsButton->setEnabled(true);
    q->EnableEffects(true);
    this->OptionsGroupBox->setEnabled(true);
    ...
    for (int i = 0; i < 3; i++) {
        ctkFlowLayout* effectsGroupLayout1 = new ctkFlowLayout();
        effectsGroupLayout1->setContentsMargins(4, 4, 4, 4);
        effectsGroupLayout1->setSpacing(4);
        effectsGroupLayout1->setAlignItems(false);
        effectsGroupLayout1->setAlignment(Qt::AlignJustify);
        effectsGroupLayout1->setPreferredExpandingDirections(Qt::Vertical);
        QGroupBox* group = EffectsGroups.at(i);
        group->setLayout(effectsGroupLayout1);
    }
    ...
    SegmentsTableResizableFrame->setVisible(false);
    AddSegmentButton->setVisible(false);
    RemoveSegmentButton->setVisible(false);
    Show3DButton->setVisible(false);
    SwitchToSegmentationsButton->setVisible(false);
    MasterVolumeNodeComboBox->setVisible(false);
    SegmentationNodeComboBox->setVisible(false);
    MasterVolumeNodeLabel->setVisible(false);
    SegmentationNodeLabel->setVisible(false);
    SpecifyGeometryButton->setVisible(false);
    SegmentsTableView->setVisible(false);
    SliceRotateWarningButton->setVisible(false);
    MaskingGroupBox->setVisible(false);
    MaskModeComboBox->setVisible(false);
    MaskModeLabel->setVisible(false);
    MasterVolumeIntensityMaskCheckBox->setVisible(false);
    MasterVolumeIntensityMaskRangeLabel->setVisible(false);
    MasterVolumeIntensityMaskRangeWidget->setVisible(false);
    OverwriteModeComboBox->setVisible(false);
    OverwriteModeLabel->setVisible(false);
    tabWidget->setTabVisible(2, false);

    //this->widget->setStyleSheet("#EffectsOptionsFrame{background-color: #363D4A;}");
    this->EffectsOptionsFrame->setStyleSheet("#widget #EffectsOptionsFrame{background-color: #363D4A;}");
	this->widget->setStyleSheet("#widget,#EffectsGroupBasic,#EffectsGroupAdvance,#EffectsGroupResearch{background-color: #363D4A;}#widget #EffectsOptionsFrame{background-color: #363D4A;}");
	
}

//-----------------------------------------------------------------------------
QToolButton* qSlicerNaviFrameWidgetPrivate::toolButton(qSlicerSegmentEditorAbstractEffect* effect)
{
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidgetPrivate::notifyEffectsOfReferenceGeometryChange(const std::string& geometry)
{
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidgetPrivate::notifyEffectsOfMasterVolumeNodeChange()
{
    foreach(qSlicerSegmentEditorAbstractEffect * effect, this->RegisteredEffects)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidgetPrivate::notifyEffectsOfLayoutChange()
{
    foreach(qSlicerSegmentEditorAbstractEffect * effect, this->RegisteredEffects)
    {
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::resetModifierLabelmapToDefault()
{
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::updateSelectedSegmentLabelmap()
{
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::updateAlignedMasterVolume()
{
    if (!this->ParameterSetNode)
    {
        qCritical() << Q_FUNC_INFO << ": Invalid segment editor parameter set node";
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::updateMaskLabelmap()
{
    if (!this->ParameterSetNode)
    {
        qCritical() << Q_FUNC_INFO << ": Invalid segment editor parameter set node";
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::updateReferenceGeometryImage()
{
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidgetPrivate::selectFirstSegment()
{
    if (!this->ParameterSetNode)
    {
        qCritical() << Q_FUNC_INFO << ": Invalid segment editor parameter set node";
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidgetPrivate::setEffectCursor(qSlicerSegmentEditorAbstractEffect* effect)
{
...
//-----------------------------------------------------------------------------
std::string qSlicerNaviFrameWidgetPrivate::getReferenceImageGeometryFromSegmentation(vtkSegmentation* segmentation)
{
    if (!segmentation)
    {
        return "";
...
//-----------------------------------------------------------------------------
std::string qSlicerNaviFrameWidgetPrivate::referenceImageGeometry()
{
    if (!this->ParameterSetNode)
    {
        qCritical() << Q_FUNC_INFO << ": Invalid segment editor parameter set node";
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::segmentationDisplayableInView(vtkMRMLAbstractViewNode* viewNode)
{
    if (!viewNode)
    {
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::segmentationIJKToRAS(vtkMatrix4x4* ijkToRas)
{
    if (!this->ParameterSetNode)
    {
        return false;
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidgetPrivate::setSurfaceSmoothingFactor(double smoothingFactor)
{
    if (!this->ParameterSetNode)
    {
        return false;
...
//-----------------------------------------------------------------------------
// qSlicerNaviFrameWidget methods

//-----------------------------------------------------------------------------
qSlicerNaviFrameWidget::qSlicerNaviFrameWidget(QWidget* _parent)
    : qMRMLWidget(_parent)
    , d_ptr(new qSlicerNaviFrameWidgetPrivate(*this))
{
    Q_D(qSlicerNaviFrameWidget);
    d->init();
}

//-----------------------------------------------------------------------------
qSlicerNaviFrameWidget::~qSlicerNaviFrameWidget() = default;

//-----------------------------------------------------------------------------
//void qSlicerNaviFrameWidget::updateEffectList()
//{
//    Q_D(qSlicerNaviFrameWidget);
//    QList<QAbstractButton*> btns = d->EffectsGroupBox->findChildren<QAbstractButton*>();
//    QList<QAbstractButton*> effectButtons1 = d->EffectButtonGroup.buttons();
//    QList<QFrame*> fras = d->EffectsOptionsFrame->findChildren<QFrame*>();
//    for (int i = 0; i < btns.size(); i++) {
//        btns.at(i)->hide();
//        d->EffectsGroupBox->layout()->removeWidget(btns.at(i));
//    }
//    for(int i = 0 ; i < effectButtons1.size(); i++){
//        d->EffectButtonGroup.removeButton(effectButtons1.at(i));
//    }
//    /*for (int i = 0; i < fras.size(); i++) {
//        d->EffectsOptionsFrame->layout()->removeWidget(fras.at(i));
//    }*/
//
//    
//
...
//   
//    QList<qSlicerSegmentEditorAbstractEffect*> tmpEffects;
//    
//    d->activeNameList.clear();
//    d->activeNameList << "Paint" << "Threshold" << "Draw" << "Erase" << "Level tracing" 
//        << "Local Threshold" << "Flood filling" << "Scissors"<<"Margin"<< "Islands" << "Hollow"
//        <<"Smoothing"<<"Surface cut";
//    auto tmpList = d->activeNameList;
//    while(d->activeNameList.length()>0){
//        bool find = false;
//        foreach(qSlicerSegmentEditorAbstractEffect * effect, d->RegisteredEffects) {
//            if (effect->m_Name == d->activeNameList[0]) {
//                d->activeNameList.pop_front();
//                d->RegisteredEffects.removeOne(effect);
//                tmpEffects.push_back(effect);
//                find = true;
//                break;
//            }
//        }
...
//        tmpEffects.push_back(effect);
//    }
//    d->RegisteredEffects = qSlicerSegmentEditorEffectFactory::instance()->copyEffects(tmpEffects);
//    d->activeNameList = tmpList;
//    //QString dd = type;
//    // Set up effect connections and options frame for all newly added effects
//    //foreach(qSlicerSegmentEditorAbstractEffect * effect, d->RegisteredEffects)
//    for(int i = 0 ; i < d->RegisteredEffects.length() ; i++)
//    {
//        auto effect = d->RegisteredEffects.at(i);
//        QObject::connect(effect, SIGNAL(canceled()), this, SLOT(onRightClickCanceled()));
...
//        }
//        effectButton->show();
//        //effectButton->setFixedSize(256, 50);
//        effectButton->setFixedSize(250, 50);
//
//        d->EffectsGroupBox->layout()->addWidget(effectButton);
//    }
//    d->UndoButton->setVisible(false);
//    d->RedoButton->setVisible(false);
//}
...
void qSlicerNaviFrameWidget::updateEffectList()
{
    ...
    {
        // Add nullptr effect (arrow button to deactivate all effects)
        QToolButton* effectButton = new QToolButton(d->EffectsGroupResearch);
        effectButton->setObjectName(NULL_EFFECT_NAME);
        effectButton->setCheckable(true);
        effectButton->setIcon(QIcon(":Icons/NullEffect.png"));
        effectButton->setText(d->EffectNameMapper["None"]);
        effectButton->setStyleSheet("font: 10pt");
        effectButton->setToolTip("No editing");
        effectButton->setToolButtonStyle(d->EffectButtonStyle);
        effectButton->setProperty("Effect", QVariant::fromValue<QObject*>(nullptr));
        effectButton->setSizePolicy(QSizePolicy::MinimumExpanding, effectButton->sizePolicy().verticalPolicy());
    ...
    {
        QString effectname = effect->m_Name;
        effect->setupOptionsFrame(); 
        ...
        effectButton->setIcon(effect->icon());
        effectButton->setStyleSheet("font: 10pt");
        if(d->EffectNameMapper.contains(effectname)){
            effectButton->setText(d->EffectNameMapper[effectname]);
        }
        else {
            effectButton->setText(effectname);
        }
        effectButton->setToolTip(effectname);
        effectButton->setSizePolicy(QSizePolicy::MinimumExpanding, effectButton->sizePolicy().verticalPolicy());
        effectButton->setProperty("Effect", QVariant::fromValue<QObject*>(effect));
        d->EffectButtonGroup.addButton(effectButton);
    ...
    {
        button->hide();
        d->EffectsGroupResearch->layout()->removeWidget(button);
    }
    
    QList<qSlicerSegmentEditorAbstractEffect*> displayedEffectsBaisc; // list of effect buttons to be displayed
    QList<qSlicerSegmentEditorAbstractEffect*> displayedEffectsAdvance; // list of effect buttons to be displayed
    QList<qSlicerSegmentEditorAbstractEffect*> displayedEffectsResearch; // list of effect buttons to be displayed
    QList<qSlicerSegmentEditorAbstractEffect*> unorderedEffects = d->RegisteredEffects;

    // Add null effect first
    displayedEffectsBaisc << nullptr;

    // Add effects in the requested order
    foreach(QString effectName, d->EffectNameOrderBasic)
    {
        qSlicerSegmentEditorAbstractEffect* effect = this->effectByName(effectName);
        if (effect)
        {
            displayedEffectsBaisc << effect;
            unorderedEffects.removeOne(effect);
        }
    }

    foreach(QString effectName, d->EffectNameOrderAdvance)
    {
        qSlicerSegmentEditorAbstractEffect* effect = this->effectByName(effectName);
        if (effect)
        {
            displayedEffectsAdvance << effect;
            unorderedEffects.removeOne(effect);
        }
    }
    ...
    // Add buttons of displayed effect to layout
    foreach(qSlicerSegmentEditorAbstractEffect * effect, displayedEffectsBaisc)
    {
        QToolButton* effectButton = d->toolButton(effect);
        if (!effectButton)
        {
        ...
        effectButton->show();
        //effectButton->setFixedSize(256, 50);
        effectButton->setFixedSize(70, 50);
		effectButton->setStyleSheet("QToolButton{color:white;font-size: 12px;font-family: Source Han Sans CN-Normal, Source Han Sans CN;font-weight: 400;}\
                                     QToolTip{background-color: #ffffff; color: #000000;}");
        d->EffectsGroups.at(0)->layout()->addWidget(effectButton);
    }

    // Add buttons of displayed effect to layout
    foreach(qSlicerSegmentEditorAbstractEffect* effect, displayedEffectsAdvance)
    {
        QToolButton* effectButton = d->toolButton(effect);
        if (!effectButton)
        {
        ...
        d->EffectsGroups.at(1)->layout()->addWidget(effectButton);
    }

    foreach(qSlicerSegmentEditorAbstractEffect* effect, unorderedEffects)
    {
        QToolButton* effectButton = d->toolButton(effect);
        if (!effectButton)
        {
		...
		effectButton->setStyleSheet("QToolButton{color:white;font-size: 12px;font-family: Source Han Sans CN-Normal, Source Han Sans CN;font-weight: 400;}\
                                     QToolTip{background-color: #ffffff; color: #000000;}");
        d->EffectsGroups.at(2)->layout()->addWidget(effectButton);
    }
}

void qSlicerNaviFrameWidget::onRightClickCanceled() {
    setActiveEffect(nullptr);
}

void qSlicerNaviFrameWidget::EnableEffects(bool b) {
    Q_D(qSlicerNaviFrameWidget);
    for (int i = 0; i < d->EffectsGroups.size(); i++) {
        QGroupBox* box = d->EffectsGroups.at(i);
        box->setEnabled(b);
    }
}

//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateWidgetFromMRML()
{
    Q_D(qSlicerNaviFrameWidget);
    ...
    if (!d->ParameterSetNode)
    {
        ...
        EnableEffects(true);
        d->MaskingGroupBox->setEnabled(true);
        d->EffectsOptionsFrame->setEnabled(true);
        d->MasterVolumeNodeComboBox->setEnabled(true);
        return;
    }
    else {
        d->SegmentationNodeComboBox->setEnabled(true);
   ...
   // this->updateWidgetFromMasterVolumeNode();
    ...
    EnableEffects(d->SegmentationNode != nullptr);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateMaskingSection()
{
    Q_D(qSlicerNaviFrameWidget);
    if (!this->mrmlScene() || this->mrmlScene()->IsClosing())
    {
    ...
    int maskModeIndex = -1;
    if (d->ParameterSetNode->GetMaskMode() == vtkMRMLSegmentEditorNode::OverwriteVisibleSegments)
    {
        // segment item
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::setMasterRepresentationToBinaryLabelmap()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateWidgetFromSegmentationNode()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
//void qSlicerNaviFrameWidget::updateWidgetFromMasterVolumeNode()
//{
//    Q_D(qSlicerNaviFrameWidget);
//
//    if (!d->ParameterSetNode)
...
//        effect->SetMasterVolumeNode(masterVolumeNode);
//    }
//
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onMasterVolumeImageDataModified()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
qSlicerSegmentEditorAbstractEffect* qSlicerNaviFrameWidget::activeEffect()const
{
    Q_D(const qSlicerNaviFrameWidget);

    return d->ActiveEffect;
}

//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setActiveEffect(qSlicerSegmentEditorAbstractEffect * effect)
{
    Q_D(qSlicerNaviFrameWidget);
    ...
    d->ParameterSetNode->SetActiveEffectName(effect ? effect->m_Name.toUtf8() : "");
}

//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateEffectsSectionFromMRML()
{
    Q_D(qSlicerNaviFrameWidget);
    ...
    EnableEffects(effectsOverallEnabled);
    ...
    if (effectsOverallEnabled)
    {
        ...
        foreach(QAbstractButton * effectButton, effectButtons)
        {
            ...
            if (d->activeNameList.contains(effect->m_Name)) {
                effectButton->setEnabled(true);
            }
            else {
                effectButton->setEnabled(true);
            }
            if(d->EffectNameOrderAdvance.contains(effect->m_Name)||
                d->EffectNameOrderBasic.contains(effect->m_Name)) {
                effectButton->setEnabled(true);
            }
            else {
    ...
    if (activeEffect)
    {
        ...
        d->OptionsGroupBox->setTitle(d->EffectNameMapper[activeEffect->m_Name]);
       ...
       // activeEffect->optionsFrame()->setMinimumHeight(activeEffect->optionsFrame()->sizeHint().height());
        activeEffect->optionsFrame()->setMaximumWidth(440);
    ...
    if (activeEffect)
    {
        effectName = activeEffect->m_Name;
    }
    QList<QAbstractButton*> effectButtons = d->EffectButtonGroup.buttons();
    foreach(QAbstractButton * effectButton, effectButtons)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setMRMLScene(vtkMRMLScene * newScene)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onMRMLSceneEndCloseEvent()
{
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onInteractionNodeModified()
{
    Q_D(const qSlicerNaviFrameWidget);
...
//------------------------------------------------------------------------------
vtkMRMLSegmentEditorNode* qSlicerNaviFrameWidget::mrmlSegmentEditorNode()const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setMRMLSegmentEditorNode(vtkMRMLSegmentEditorNode * newSegmentEditorNode)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
vtkMRMLInteractionNode* qSlicerNaviFrameWidget::interactionNode()const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setInteractionNode(vtkMRMLInteractionNode * interactionNode)
{
    Q_D(qSlicerNaviFrameWidget);
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::initializeParameterSetNode()
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
...
vtkMRMLNode* qSlicerNaviFrameWidget::getClonedNode() {
    Q_D(qSlicerNaviFrameWidget);
    return d->ClonedNode;
}

void qSlicerNaviFrameWidget::setClonedNode(vtkMRMLNode* node) {
    Q_D(qSlicerNaviFrameWidget);
    d->ClonedNode = node;
}

//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setSegmentationNode(vtkMRMLNode * node)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
vtkMRMLNode* qSlicerNaviFrameWidget::segmentationNode()const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setSegmentationNodeID(const QString & nodeID)
{
    Q_D(qSlicerNaviFrameWidget);
    if (!this->mrmlScene())
    {
...
//------------------------------------------------------------------------------
QString qSlicerNaviFrameWidget::segmentationNodeID()const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
QString qSlicerNaviFrameWidget::currentSegmentID()const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setMasterVolumeNode(vtkMRMLNode * node)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
vtkMRMLNode* qSlicerNaviFrameWidget::masterVolumeNode()const
{
    Q_D(const qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setMasterVolumeNodeID(const QString & nodeID)
{
    Q_D(qSlicerNaviFrameWidget);
    if (!this->mrmlScene())
    {
...
//------------------------------------------------------------------------------
QString qSlicerNaviFrameWidget::masterVolumeNodeID()const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSegmentationNodeChanged(vtkMRMLNode * node)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSegmentSelectionChanged(const QItemSelection & selected, const QItemSelection & deselected)
{
    Q_UNUSED(selected);
    Q_UNUSED(deselected);
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setCurrentSegmentID(const QString segmentID)
{
    Q_D(qSlicerNaviFrameWidget);
    if (d->ParameterSetNode)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onMasterVolumeNodeChanged(vtkMRMLNode * node)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onEffectButtonClicked(QAbstractButton * button)
{
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onAddSegment()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onRemoveSegment()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
qSlicerAbstractModuleWidget* qSlicerNaviFrameWidget::switchToSegmentationsModule()
{
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSwitchToSegmentations()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onCreateSurfaceToggled(bool on)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSegmentAddedRemoved()
{
    Q_D(qSlicerNaviFrameWidget);
    ...
    {
        d->Show3DButton->setEnabled(true);
        d->SwitchToSegmentationsButton->setEnabled(true);
    }
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::showMasterVolumeInSliceViewers(bool forceShowInBackground /*=false*/, bool fitSlice /*=false*/)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onLayoutChanged(int layoutIndex)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
qSlicerSegmentEditorAbstractEffect* qSlicerNaviFrameWidget::effectByName(QString name)
{
    Q_D(qSlicerNaviFrameWidget);
    ...
    {
        if (currentEffect->m_Name.compare(name) == 0)
        {
...
//------------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::unorderedEffectsVisible() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
vtkMRMLSegmentationNode* qSlicerNaviFrameWidget::AddNewSegmentationNode(QString label) {
    Q_D(qSlicerNaviFrameWidget);
    auto segmentationNode = mrmlScene()->CreateNodeByClass("vtkMRMLSegmentationNode");
    segmentationNode->SetAttribute("structure_name", label.toStdString().c_str());
    mrmlScene()->AddNode(segmentationNode);
    d->SegmentationNodeComboBox->setCurrentNode(segmentationNode);
    vtkMRMLSegmentationNode* segNode = vtkMRMLSegmentationNode::SafeDownCast(segmentationNode);
    setSegmentationNode(segNode);
    onCreateSurfaceToggled(true);
    return vtkMRMLSegmentationNode::SafeDownCast(segmentationNode);
}

void qSlicerNaviFrameWidget::SetMasterNode(vtkMRMLNode* node) {
    Q_D(qSlicerNaviFrameWidget);
    setMasterVolumeNode(node);
    d->MasterVolumeNodeComboBox->setCurrentNode(node);
    installKeyboardShortcuts();
}

//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setUnorderedEffectsVisible(bool visible)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
int qSlicerNaviFrameWidget::effectCount()
{
    Q_D(qSlicerNaviFrameWidget);
    int n = 0;
    for (int i = 0; i < d->EffectsGroups.size(); i++) {
        n += d->EffectsGroups.at(i)->layout()->count();
    }
    return n;
}

//---------------------------------------------------------------------------
qSlicerSegmentEditorAbstractEffect* qSlicerNaviFrameWidget::effectByIndex(int index)
{
    return nullptr;
}

//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setupViewObservations()
{
    Q_D(qSlicerNaviFrameWidget);
    ...
    foreach(QString sliceViewName, layoutManager->sliceViewNames())
    {
        ...
        interactionCallbackCommand->SetCallback(qSlicerNaviFrameWidget::processEvents);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::removeViewObservations()
{
    Q_D(qSlicerNaviFrameWidget);
...
void qSlicerNaviFrameWidget::selectFirstSegment() {
    Q_D(qSlicerNaviFrameWidget);
    d->selectFirstSegment();
}
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setActiveEffectByName(QString effectName)
{
    qSlicerSegmentEditorAbstractEffect* effect = this->effectByName(effectName);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::saveStateForUndo()
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateVolume(void* volumeToUpdate, bool& success)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::processEvents(vtkObject * caller,
    unsigned long eid,
    void* clientData,
    void* vtkNotUsed(callData))
{
    ...
    qSlicerNaviFrameWidget* self = callbackCommand->EditorWidget.data();
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onMasterVolumeIntensityMaskChecked(bool checked)
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onMasterVolumeIntensityMaskRangeChanged(double min, double max)
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onMaskModeChanged(int index)
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
    ...
    if (index >= d->MaskModeComboBoxFixedItemsCount)
    {
        ...
        d->ParameterSetNode->SetMaskMode(vtkMRMLSegmentEditorNode::OverwriteVisibleSegments);
    }
    else
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onOverwriteModeChanged(int index)
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::segmentationNodeSelectorVisible() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setSegmentationNodeSelectorVisible(bool visible)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::masterVolumeNodeSelectorVisible() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setMasterVolumeNodeSelectorVisible(bool visible)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::switchToSegmentationsButtonVisible() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setSwitchToSegmentationsButtonVisible(bool visible)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::undoEnabled() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setUndoEnabled(bool enabled)
{
    Q_D(qSlicerNaviFrameWidget);
    if (enabled)
    {
...
//-----------------------------------------------------------------------------
int qSlicerNaviFrameWidget::maximumNumberOfUndoStates() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setMaximumNumberOfUndoStates(int maxNumberOfStates)
{
    Q_D(qSlicerNaviFrameWidget);
...
//------------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::readOnly() const
{
    Q_D(const qSlicerNaviFrameWidget);
    return d->Locked;
}
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setReadOnly(bool aReadOnly)
{
    Q_D(qSlicerNaviFrameWidget);
    d->Locked = aReadOnly;
    if (aReadOnly)
...
//------------------------------------------------------------------------------
void qSlicerNaviFrameWidget::toggleMasterVolumeIntensityMaskEnabled()
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->ParameterSetNode)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::undo()
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->SegmentationNode)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::redo()
{
    Q_D(qSlicerNaviFrameWidget);
    if (!d->SegmentationNode)
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSegmentationHistoryChanged()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::installKeyboardShortcuts(QWidget * parent /*=nullptr*/)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::uninstallKeyboardShortcuts()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSelectEffectShortcut()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSelectSegmentShortcut()
{
    Q_D(qSlicerNaviFrameWidget);
    QShortcut* shortcut = qobject_cast<QShortcut*>(sender());
    if (shortcut == nullptr)
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::hideLabelLayer()
{
    Q_D(qSlicerNaviFrameWidget);
    qSlicerLayoutManager* layoutManager = qSlicerApplication::application()->layoutManager();
    if (!layoutManager)
...
//---------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::turnOffLightboxes()
{
    Q_D(qSlicerNaviFrameWidget);
    qSlicerLayoutManager* layoutManager = qSlicerApplication::application()->layoutManager();
    if (!layoutManager)
...
//---------------------------------------------------------------------------
Qt::ToolButtonStyle qSlicerNaviFrameWidget::effectButtonStyle() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setEffectButtonStyle(Qt::ToolButtonStyle toolButtonStyle)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::segmentationNodeSelectorAddAttribute(const QString & nodeType,
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::segmentationNodeSelectorRemoveAttribute(const QString & nodeType,
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::masterVolumeNodeSelectorAddAttribute(const QString & nodeType,
    const QString & attributeName, const QVariant & attributeValue/*=QVariant()*/)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::masterVolumeNodeSelectorRemoveAttribute(const QString & nodeType,
    const QString & attributeName)
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::anchorClicked(const QUrl & url)
{
    if (url.path().isEmpty())
    {
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateEffectLayouts()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onEnableSurfaceSmoothingToggled(bool smoothingEnabled)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSurfaceSmoothingFactorChanged(double newSmoothingFactor)
{
    Q_D(qSlicerNaviFrameWidget);
    // Get segmentation node
    vtkMRMLSegmentationNode* segmentationNode = nullptr;
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onImportExportActionClicked()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onExportToFilesActionClicked()
{
    Q_D(qSlicerNaviFrameWidget);
...
//-----------------------------------------------------------------------------
void qSlicerNaviFrameWidget::onSegmentationDisplayModified()
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::autoShowMasterVolumeNode() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setAutoShowMasterVolumeNode(bool autoShow)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::updateSliceRotateWarningButtonVisibility()
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::rotateSliceViewsToSegmentation()
{
    Q_D(qSlicerNaviFrameWidget);
    vtkNew<vtkMatrix4x4> segmentationIJKToRAS;
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::showSegmentationGeometryDialog()
{
    Q_D(qSlicerNaviFrameWidget);
    return;
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::selectPreviousSegment()
{
    this->selectSegmentAtOffset(-1);
}

//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::selectNextSegment()
{
    this->selectSegmentAtOffset(1);
}

//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::selectSegmentAtOffset(int offset)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
QString qSlicerNaviFrameWidget::defaultTerminologyEntry()
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setDefaultTerminologyEntry(const QString & terminologyEntry)
{
    Q_D(qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
QString qSlicerNaviFrameWidget::defaultTerminologyEntrySettingsKey() const
{
    Q_D(const qSlicerNaviFrameWidget);
...
//---------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setDefaultTerminologyEntrySettingsKey(const QString & terminologyEntrySettingsKey)
{
    Q_D(qSlicerNaviFrameWidget);
...
// --------------------------------------------------------------------------
void qSlicerNaviFrameWidget::jumpSlices()
{
    Q_D(qSlicerNaviFrameWidget);
...
// --------------------------------------------------------------------------
void qSlicerNaviFrameWidget::setJumpToSelectedSegmentEnabled(bool enable)
{
    Q_D(qSlicerNaviFrameWidget);
...
// --------------------------------------------------------------------------
bool qSlicerNaviFrameWidget::jumpToSelectedSegmentEnabled()const
{
    Q_D(const qSlicerNaviFrameWidget);
